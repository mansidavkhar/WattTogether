...
exports.register = async (req, res) => {
    ...
    console.log("Request Body:", req.body); // Log the data received from the frontend
    ...
    try {
        ...
        // --- Enhanced Logging: Step 4 (Crucial Error Catcher) ---
        console.error("!!! REGISTRATION ERROR !!!:", error);
        res.status(500).json({ success: false, message: "Server Error during registration." });
    }
};

// --- Member Login (No changes needed, but kept for completeness) ---
exports.login = async (req, res) => {
    const { email, password } = req.body;
    try {
        const member = await Member.findOne({ email });
...
// --- Save Wallet Address (No changes needed) ---
exports.saveWalletAddress = async (req, res) => {
    try {
        const { walletAddress } = req.body;
        const memberId = req.member.id; 
